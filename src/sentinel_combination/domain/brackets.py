...
class BracketPlan:
    ...
    def validate_against_entry(self, entry_price: Decimal) -> None:
        ...
        if self.entry_side is Side.BUY:
            ...
            if any(
                later.trigger_price <= earlier.trigger_price
                for earlier, later in zip(self.take_profit_targets, self.take_profit_targets[1:])
            ):
    ...
    def compile_exit_intents(
        ...
        *,
        ...
        if native_oco and self.take_profit_targets:
            allocated = Decimal("0")
            for index, target in enumerate(self.take_profit_targets):
                quantity = instrument.quantize_quantity_down(
                    confirmed * target.close_fraction
                )
                if quantity <= 0:
                    raise ValueError(f"target {index} quantity rounds to zero")
                allocated += quantity
                oca = f"{self.bracket_id}:slice:{index}"
                intents.extend(
                    [
                        OrderIntent(
                            client_order_id=f"{self.bracket_id}:stop:{index}",
                        ...
                        OrderIntent(
                            ...
                            limit_price=instrument.quantize_price(
            ...
            runner = instrument.quantize_quantity_down(confirmed - allocated)
            if runner > 0:
                intents.append(
                    OrderIntent(
                        client_order_id=f"{self.bracket_id}:stop:runner",
                        account_id=self.account_id,
                        instrument_id=self.instrument_id,
                        side=exit_side,
                        quantity=runner,
        ...
        # On reduce-only derivatives venues, one durable full-size stop remains
        # valid as targets reduce exposure; the venue must prevent reversal.
        intents.append(
            OrderIntent(
